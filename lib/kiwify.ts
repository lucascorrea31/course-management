...
const KIWIFY_API_BASE = "https://public-api.kiwify.com.br";
...
class KiwifyClient {
  ...
  private async request<T>(
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Kiwify API Error: ${response.status} - ${error || response.statusText}`
      );
    }
...
export function getKiwifyClient(): KiwifyClient {
  if (!kiwifyClient) {
    ...
    if (!apiKey || !account) {
      throw new Error(
        "KIWIFY_API_KEY e KIWIFY_ACCOUNT devem estar definidos no .env.local"
      );
    }
